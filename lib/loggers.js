...
// class to handle pretty printing module name
class ModuleFileTransport extends winston.transports.Console {
  constructor(config) {
    super(config);
    this.modulePrefix = config.bedrock.modulePrefix;
  }
...
container.init = function(callback) {
  if(cluster.isMaster) {
    ...
    transports.app = new ModuleConsoleTransport(config.loggers.console);
    transports.access = new ModuleConsoleTransport(config.loggers.console);
    transports.error = new ModuleConsoleTransport(config.loggers.console);
    transports.email = new ModuleConsoleTransport(config.loggers.console);
